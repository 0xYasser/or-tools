...
$(DOTNET_ORTOOLS_NATIVE_LIBS): \
 ...
 $(GEN_DIR)/ortools/sat/CpModel.pb.cs \
 | $(PACKAGE_DIR) $(BIN_DIR)
	"$(DOTNET_BIN)" build -r $(RUNTIME_IDENTIFIER) ortools$Sdotnet$S$(OR_TOOLS_NATIVE_ASSEMBLY_NAME)$S$(OR_TOOLS_NATIVE_ASSEMBLY_NAME).csproj
ifeq ($(SYSTEM),win)
	$(COPY) ortools$Sdotnet$S$(OR_TOOLS_NATIVE_ASSEMBLY_NAME)$Sbin$Sx64$SDebug$Snetstandard2.0$S$(RUNTIME_IDENTIFIER)$S*.* $(BIN_DIR)
else
	$(COPY) ortools$Sdotnet$S$(OR_TOOLS_NATIVE_ASSEMBLY_NAME)$Sbin$SDebug$Snetstandard2.0$S$(RUNTIME_IDENTIFIER)$S*.* $(BIN_DIR)
endif
